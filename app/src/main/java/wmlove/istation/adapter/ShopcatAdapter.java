...
import wmlove.istation.ShopActivity;
...
public class ShopcatAdapter extends BaseExpandableListAdapter {
    ...
    public View getGroupView(final int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        groupViewHolder.storeName.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("shopId",group.getShopId());
         ...
         */
        if (group.isEditor()) {
            groupViewHolder.storeEdit.setImageResource(R.drawable.shopping_cart_list_check_ok);
        } else {
            groupViewHolder.storeEdit.setImageResource(R.drawable.shopping_cart_list_check);
        }
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if (child != null) {
            childViewHolder.goodsImage.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(mcontext, GoodDetailActivity.class);
                    intent.putExtra("goodID", child.getId());
                    mcontext.startActivity(intent);
                }
            });
            ...
            Glide.with(mcontext).load(Common.API_IMAGE_DOMAIN_URL + child.getImageUrl().split("www/")[1]).into(childViewHolder.goodsImage);
            String desc = child.getDesc().length() > 18 ? child.getDesc().substring(0,18)  + "..." : child.getDesc();
            childViewHolder.goods_size.setText(desc + "\n类型:" + child.getSize());
            childViewHolder.goodsSize.setText(desc + "\n类型:" + child.getSize());
            //设置打折前的原价
...
//            childViewHolder.goodsPrimePrice.setText(spannableString);
    ...
    private class GroupViewClick implements View.OnClickListener {
        ...
        public GroupViewClick(StoreInfo group, int groupPosition, ImageView editor) {
            this.group = group;
